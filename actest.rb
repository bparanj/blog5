...
class BugTest < Minitest::Test
  ...
  def test_wrapping_unlisted_attribute
    post '/todos.json', { content_html: "<b>Please clap</b>" }.to_json, { "CONTENT_TYPE" => "application/json" }
    assert last_response.ok?, "Wrapping did not work when enabled for all controllers based on format"
  end
  ...
  def test_explicitly_wrapping_unlisted_attribute
    ...
    assert last_response.ok?, "Wrapping did not work when given explicitly with include option"
  end
